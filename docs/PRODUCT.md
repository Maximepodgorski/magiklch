...
**One-liner:** Generate production-ready OKLCH palettes with APCA contrast scoring, NxN shade contrast grid, gamut warnings, and 1-click copy.
...
|---------|------|-------------|
| **Design Engineer** | Generate Tailwind-compatible palettes fast | Input hue → get shades, copy CSS variables |
...
|---|-------|-------------------|
| US-1 | As a user, I can enter a base color (HEX, OKLCH, or HSL) and get a palette with configurable shade count (4–12 shades, steps 50–975) | Input field accepts multiple formats; generates shades instantly |
...
| US-10 | As a user, I can see an NxN APCA contrast grid for all shade pairs | Contrast tab in Generator shows every text/bg combination with Lc values and pass/fail badges |

### Should-Have (V1.0)

| # | Story | Acceptance criteria |
|---|-------|-------------------|
| US-11 | As a user, I can control Lightness, Chroma, and Hue via dedicated slider bars | 3 range sliders (L: 0–100%, C: 0–0.4, H: 0–360°) with numeric input fields; real-time palette update |
| US-12 | As a user, I can see a P3 gamut indicator alongside sRGB | Dual gamut badges on ShadeCard |
| US-13 | As a user, I can navigate the entire UI with keyboard | Full keyboard nav, visible focus indicators |
| US-14 | As a user, I see shade cards with just the color swatch and shade number — OKLCH values appear on hover as a tooltip | Clean minimal cards; hover/focus triggers tooltip with OKLCH/HEX values |
| US-15 | As a user, I can copy the entire palette in multiple output formats (CSS variables, SCSS variables) | Format selector + "Copy all" button; outputs production-ready code blocks |

### V1.1 — Claude Code Skill

| # | Story | Acceptance criteria |
|---|-------|-------------------|
| US-16 | As a Claude Code user, I can run `/oklch <color>` to generate a palette in terminal | Skill outputs formatted palette in terminal |
| US-17 | As a Claude Code user, I can run `/oklch --css` to get copy-pasteable CSS variables | Outputs `:root { --color-50: ... }` block |
...
- **Shade generation** — 4–12 shades (50–975 range, configurable scale) with non-linear lightness curve and chroma bell curve
- **Palette display** — Clean grid of ShadeCards showing swatch + shade number only; OKLCH/HEX values revealed on hover via tooltip
- **APCA contrast** — Real-time Lc values via `apca-w3`, badges (AAA/AA/A/Fail) against white and black. NxN contrast grid for all shade pairs.
...
- **Multi-format export** — Copy entire palette as CSS variables or SCSS variables

### Routes

| Route | Purpose |
|-------|---------|
| `/` | Generator — seed color, L/C/H sliders, scale, gamut, Shades/Contrast tabs, export |
